# 만물인테리어 · Loop Agent

**AI가 24시간 운영하는 만물인테리어 회사 웹사이트**

상담부터 견적, 시공 관리, 사후 관리까지 — AI 루프 에이전트가 순환하며
운영하는 인테리어 회사 콘셉트의 반응형 원페이지 웹사이트입니다.

기획서(`AI 운영형 홈페이지 기획서`)의 구조 —
**웹 → n8n(워크플로) → 카카오톡(알림·챗봇) → 대표 승인** — 을 그대로 구현했습니다.

벤치마킹 비율 — **3.3디자인스페이스 운영구조 40% · 1204디자인 견적 30% · 아파트멘터리 화면 25% · 인테리어티쳐 프리미엄 5%**.
겉은 깔끔·단순하게, 내부는 **견적·고객·현장·사진·A/S 데이터가 연결되는 업무 시스템**으로 구성했습니다.

**운영 4원칙**

1. **유실 0** — 접수된 건 사라지지 않는다
2. **무승인 발송 0** — 사람 승인 없이는 아무것도 나가지 않는다
3. **개발 0 운영** — 내용을 바꾸는 데 코드가 필요 없다(데이터 분리)
4. **돈은 승인·기록을 거친다** — 계약금·중도금·잔금·추가공사 모두 승인 후 기록

## 🚀 지금 실서비스로 전환하기 (대표님 우선순위)

현재는 **데모 모드**(`data/config.json`의 `demoMode: true`)입니다. 아래 순서로 켜면 됩니다.
각 단계는 독립적이라 **1번만 해도 상담이 실제로 대표님께 도착**합니다.

**1️⃣ 상담이 이메일로 오게 — 가장 빠름 · 약 2분 · 무료**
Web3Forms에 이메일만 넣고 `access_key`를 발급(회원가입 없음)받아 저(AI)에게 알려주시면,
`config.json`에 넣고 `demoMode`를 끕니다. 그 순간부터 홈페이지 상담이 **대표 이메일로 자동 도착**합니다.
현재 코드 그대로 동작하므로 서버·배포가 필요 없습니다. → 상세 [`integrations/INTEGRATION.md`](integrations/INTEGRATION.md) §3-1

**2️⃣ 검색에 노출되게 — 유입 · 무료**
네이버 스마트플레이스(지도)·서치어드바이저 등록. 동네 손님 대부분이 여기서 옵니다.
등록 순서 [`integrations/GROWTH.md`](integrations/GROWTH.md) · 실제 검색 데이터 기반 콘텐츠·복붙 초안 [`integrations/CONTENT-PLAN.md`](integrations/CONTENT-PLAN.md)

**3️⃣ 자동화 고도화 — 선택 · 무료**
n8n 무료 워크플로(규칙엔진 + Google Sheets)로 리드 저장·알림·대표 승인·카카오 챗봇까지.
설치 [`integrations/SETUP-n8n-kakao.md`](integrations/SETUP-n8n-kakao.md) · 버전 선택 [`integrations/n8n/README.md`](integrations/n8n/README.md)

> 진행 상태는 관리자 화면(`admin.html`)의 **연동 상태** 패널에서 언제든 확인·테스트할 수 있습니다.

## 구조: 공개 사이트 ↔ 운영 앱 (역할 분리)

- **manmool (이 저장소)** = **대외 공개 홈페이지** — 홍보·시공사례·AI 예상견적·상담 유입 + 웹 리드 관리.
- **[hyeonjang](https://github.com/01023978629/hyeonjang) ("현장.")** = **대내 운영 앱** — 사장님이 현장·견적·정산·A/S·회계를 관리하는 단일 파일 PWA(별도 저장소).
- **연결:** 상담 리드는 폼에서 **바로 외부 접수 경로(Web3Forms 또는 n8n)로 전송**되어 대표 이메일로 들어옵니다. 운영 앱 진입점 주소는 `data/config.json`의 `hyeonjang.appUrl` 하나로 제어합니다(상단 유틸리티 바·푸터·데모 배너 공통 — `js/hj-link.js`).
  - 관리자 화면에서 리드를 `#lead=` 딥링크로 운영 앱에 넘기던 **"🏗 현장 앱으로 보내기" 버튼은 지금 없습니다.** 리드 개인정보를 브라우저에 보관하지 않도록 바꾸면서 함께 내렸습니다(관리자 화면은 접수 경로 상태 확인과 콘텐츠 편집만 합니다). 운영 앱 쪽 `?lead=` 인입 경로는 남아 있지만, 이 저장소에서 그리로 보내는 화면은 없습니다 — 다시 열려면 만들어야 하는 기능이지 설정으로 켜는 것이 아닙니다.
- manmool의 `field.html`·`as.html`은 **미리보기 데모**로 유지하되, 실제 운영은 운영 앱으로 안내합니다.

## 홈페이지 (고객 화면)

- **시공 사례 (아파트멘터리형)** — 지역·아파트명·평수·공사비·공간종류·스타일·공사범위·공사연도 구조화 필터, 아파트명 검색, **Before/After 상세**, 자재·핵심문제·해결방법, **유사 사례 추천** (AI 사진 자동분류 안내)
- **대화식 AI 예상견적** — 공간→평수→전체·부분→예산→사진을 대화로 묻고, **예상금액·필요공사·추천 자재등급·예상 공사기간·비슷한 사례·상담 전 준비**를 생성. 결과에 **다음 단계 안내(예상견적 → 방문 실측 → 정식 견적서 → 계약)** 와 "실측 후 금액 확정" 고지
- **AI 인테리어 시뮬레이터** — 평형·준공연도·거주여부 → 공사할 공간 선택 → **공간 사진 위 세부 작업(도배·방수·타일·TV장 등)을 눌러 켜고 끄면 예상 금액이 바로 바뀜**. 예산 상한을 넘으면 무엇을 빼면 되는지 제안. 계산은 `DesignBom` 한 곳만 쓰고(사례 카탈로그와 같은 값), 금액은 범위·만원 단위로만 말한다. 결과는 `#sim=` 링크·PNG 저장·상담 폼 프리필로 이어짐
- **우리집 한 채로 보기** — 추천 디자인 섹션의 보기 모드(탭). 우리집에 있는 공간마다 시안을 한 장씩 놓아 한 채로 비교하고, 시안 교체·`#look=` 재현 링크·시뮬레이터로 범위 전달. **금액을 다시 계산하지 않고, 조합을 판정하지 않는다**
- **면허·보증·A/S 신뢰 섹션** — 고액 계약 신뢰 확보
- **체계적·효율적 운영 프로세스 = n8n·카카오톡** — 자동화 파이프라인 + 효율 지표 시각화
- **단계형 AI 상담 문의** → n8n 웹훅 · **카카오톡 챗봇** (평수·예산·스타일 슬롯 채우기)
- **검색 노출 · 공유 미리보기** — Open Graph/Twitter 카드 + `og-image.png`(카카오톡 공유 시 브랜드 미리보기), JSON-LD 구조화 데이터, `sitemap.xml`·`robots.txt`

## 업무 시스템 (내부 화면)

| 페이지 | 내용 |
|--------|------|
| `admin.html` 관리자 | 리드·AI 요약·우선순위·**승인/보류/거절** (사람 승인 장치) |
| `mypage.html` 고객 전용 링크 페이지 | **본인확인(휴대폰 뒷4자리)·최초 동의** 후 입장 → 일정·오늘의 공정·**공개 현장사진**·**계약서 전자 서명**·자재선택·**변경/추가공사 승인·기록**·**대금 흐름(계좌 안내→입금 확인·기록)**·**국가기준 하자보증(방수 3년·설비 2년·마감 1년)**·문서·카카오톡 알림 이력·준공 체크리스트·**완공 후 후기/소개** |
| `field.html` 현장관리 | 오늘 할 작업·작업 전후 사진·자재 입고·문제 보고(고위험은 사람 확인)·추가공사 요청·출입 기록·완료 체크 |
| `as.html` A/S 센터 | 하자 위치·사진 접수 → **유형 자동분류·긴급도 판단·담당 배정·방문 일정**·고객 확인 |

> 모든 화면은 **프로젝트 단위**로 데이터가 연결됩니다 (`data/project.json`). 금액·계약·발송은 대표/고객 승인 후에만 진행합니다.

## 구조

```
index.html                              홈 (고객 화면)
blog.html                               인사이트(블로그) — 목록·글·글별 SEO
admin.html / mypage.html                관리자 / 고객 마이페이지
field.html / as.html                    현장관리 / A/S 센터
css/styles.css                          홈 스타일
css/admin.css / css/app.css             관리자 / 업무 시스템 공통 스타일
js/main.js                              홈 렌더링(사례·신뢰·자동화)
js/estimate.js                          대화식 AI 예상견적
js/simulator.js                         AI 인테리어 시뮬레이터(우리집 사양서)
js/lookbook.js                          우리집 한 채로 보기(추천 디자인 보기 모드)
js/design-bom.js                        공간별 자재 산출·예상비용 엔진(시안·시뮬레이터 공용)
js/inquiry.js                           단계형 상담 폼 → n8n 전송
js/blog.js                              인사이트 목록·글 렌더(글별 canonical/OG)
js/hj-link.js                           운영 앱(hyeonjang) 링크 배선
js/admin.js / js/mypage.js              관리자 / 마이페이지
js/field.js / js/as.js                  현장관리 / A/S 센터
data/site.json                          홈 콘텐츠(회사·서비스·사례·견적)
data/project.json                       프로젝트 데이터(계약·보증·결제·일정·사진·A/S·후기)
data/config.json                        연동 설정(n8n 웹훅·카카오 채널·운영 앱 URL)
og-image.png                            공유 미리보기 이미지(1200×630)
sitemap.xml / robots.txt                검색 노출
integrations/INTEGRATION.md             n8n·카카오 연동 가이드
integrations/n8n/*.workflow.json        n8n 워크플로(import용)
.github/workflows/deploy-pages.yml      GitHub Pages 자동 배포
```

## n8n · 카카오톡 연동

1. `integrations/n8n/`의 워크플로를 n8n에 **Import** 합니다. **1인 운영이면 무료 lite 버전을 켜세요.**
   - ⭐ `manmul-inquiry-lite.workflow.json` — **무료**(규칙엔진 + Google Sheets). API 키·DB 불필요
   - `kakao-chatbot-skill.workflow.json` — 카카오 i 오픈빌더 챗봇 스킬서버
   - 〈고급〉 `manmul-inquiry.workflow.json` — Claude 요약 + Postgres (문의량 많을 때). lite와 **동시 활성 금지**(웹훅 경로 동일)
2. `data/config.json`에 n8n 웹훅 URL과 카카오 채널 URL을 넣고 `enabled: true`, `demoMode: false`로 전환합니다.
3. 자세한 절차는 [`integrations/INTEGRATION.md`](integrations/INTEGRATION.md) 참고.
4. **n8n을 아직 안 띄웠다면** — 무료 클라우드부터 처음부터 세우는
   [`integrations/SETUP-n8n-kakao.md`](integrations/SETUP-n8n-kakao.md)를 먼저 보세요.
   전환 상태는 관리자 화면(`admin.html`)의 **연동 상태** 패널에서 확인·테스트할 수 있습니다.

> **안전장치:** 금액·계약·고객 발송은 **대표 승인 후에만** 실행됩니다(초안·발송 분리).
> AI는 요약·분류·초안까지만 담당합니다.

## 콘텐츠 운영 (AI/운영자용)

- 사이트 콘텐츠는 **`data/site.json`**, 연동 설정은 **`data/config.json`** 하나로 관리됩니다.
- 두 파일만 수정하면 코드 변경 없이 사이트·연동이 갱신되므로, AI 에이전트가 운영할 수 있습니다.
- 검증기(전부 `node scripts/<파일>` 로 실행, 실패 시 exit 1):
  `ensure-site-integrity` · `ensure-conversion-basics`(접수 경로·점진 렌더·시안 공유주소 포함) ·
  `ensure-lead-route-parity` ·
  `ensure-simulator-honesty`(시뮬레이터 고지·범위표기·게이팅 부재) ·
  `ensure-lookbook-honesty`(보기 모드 고지·금액 미계산·판정 금지·사진 겹침 한도·imageAlt) ·
  `ensure-design-catalog-coverage` · `ensure-space-catalog-coverage` ·
  `verify-space-cost-estimates`(시안 300개·예상비용 불변)
  진단 도구(검증기 아님): `report-photo-duplicates`(사진 겹침 현황, `--ids` 로 대상 목록)
- **예외 1가지**: `insights`(블로그 글)를 수정했으면 `python3 scripts/prerender-posts.py`를
  실행해 `posts/` 정적 페이지·`rss.xml`·`data/leak-case-index.json`을 재생성한 뒤 함께 커밋하세요. (네이버 등 JS 렌더링이
  불안정한 검색엔진이 글을 개별 색인하도록 글마다 정적 페이지를 둡니다 — sitemap도 이 주소 기준)

## 로컬 실행

`fetch`로 JSON을 불러오므로 로컬 서버가 필요합니다.

```bash
python3 -m http.server 8000
# http://localhost:8000 접속
```

## 배포

빌드 과정이 없는 정적 사이트입니다. `main` 브랜치에 반영되면
`.github/workflows/deploy-pages.yml`이 **GitHub Pages로 자동 배포**합니다.

- 공개 사이트: **https://01023978629.github.io/manmool/**
- 운영 앱(현장.): **https://01023978629.github.io/hyeonjang/**

---

> 본 프로젝트는 AI 루프 에이전트가 콘텐츠와 상담을 운영하는 콘셉트의 데모입니다.
> 표시된 견적·통계는 예시 값이며 실제 견적은 현장 실측 후 확정됩니다.
