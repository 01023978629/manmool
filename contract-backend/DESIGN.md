...
서명 완료 시 서버는 **15분 view 토큰**을 함께 반환하고, 서명 이미지(PNG)를 `signatures.image_data`에
파생 보관한다(진위는 `image_sha256`로 검증). 고객 화면은 이 토큰을 이 탭의 sessionStorage에만
보관해, 완료 직후 **새로고침해도** `GET /api/sign/completed`로 완료 계약서(본문·서명 이미지·해시)를
다시 볼 수 있다. 장기 공개 URL은 없고(15분), 토큰은 URL에 남기지 않는다.
...
- ~~운영자 API 인증~~ → **완료**: `/api/sign*`·`/healthz` 외 운영자 라우트(계약 생성·잠금·서명링크·발송·증거)는
  `x-admin-token`(ADMIN_TOKEN) 필수. 세션/mTLS 등 더 강한 인증은 규모에 따라 확장 가능.
