(() => {
  ...
  function dataFromForm(form) { const source = form && typeof form.elements === 'object' ? form : createForm; const get = (name) => source.elements.namedItem(name); return { unit: get('unit').value, location: get('location').value, issueType: get('issueType').value, pipeType: get('pipeType').value, urgency: get('urgency').value, description: get('description').value, officeContactName: get('officeContactName').value, officeContactPhone: get('officeContactPhone').value, residentName: get('residentName').value, residentPhone: get('residentPhone').value, residentInformed: !!(get('residentInformed') && get('residentInformed').checked), preferredVisitDate: get('preferredVisitDate').value, privacyConsent: get('privacyConsent').checked }; }
  // 오류 칸이 접힌 <details>(입주민 연락처) 안이면 먼저 펼친다 — 닫힌 채로는 focus() 가 무효라
  // 포커스가 body 로 떨어지고, 직원은 화면 맨 아래 문구만 보고 어디를 고칠지 못 찾는다.
  function focusField(field) { const name = field === 'residentContact' ? 'residentName' : field; const input = createForm && createForm.elements.namedItem(name); if (!input || typeof input.focus !== 'function') return; const folded = typeof input.closest === 'function' ? input.closest('details') : null; if (folded && !folded.open) folded.open = true; input.focus(); }
