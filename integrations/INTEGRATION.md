...
- `demoMode` 값은 문의 저장 폴백을 켜지 않습니다. 실제 자동 접수 여부는 아래의 `n8n` 또는 명시 지원 `forms` 설정으로만 결정됩니다. 고객 문의는 `localStorage`에 백업하지 않으며, 자동 전송 실패 시에는 최신 문의 1건만 현재 탭 메모리에 남고 새로고침하거나 탭을 닫으면 사라집니다.
...
1. **채우기** — `data/config.json`에 실제 값 입력
   ...
   - `n8n.enabled` = `true` (`demoMode`는 접수 경로·저장 여부 검증 조건으로 사용하지 않음)
   - `kakao.channelAddUrl`·`chatUrl` = 카카오톡 채널 홈/1:1 채팅 URL
2. **검증** — 관리자 화면(`admin.html`) → **연동 상태** 패널에서
   - 배지가 n8n 또는 지원 폼 서비스 **연결됨**인지 확인
   - **접수 경로 설정 확인** 버튼으로 설정 조건을 확인합니다. 이 버튼은 실제 endpoint로 요청을 보내거나 시험 문의를 만들지 않습니다.
   - 실제 수신 검증이 필요하면 승인된 시험 데이터로 상담 폼을 1회 제출하고, n8n 또는 폼 서비스의 외부 수신 화면에서 별도로 확인합니다.
   (n8n Webhook 노드는 웹 도메인에 대해 **CORS 허용** 필요)
3. **비밀값 분리** — 관리자 토큰·API 키 등은 **repo에 넣지 말고** n8n
   환경변수(`KAKAO_ADMIN_TOKEN`, `ANTHROPIC_API_KEY` 등)로 관리

---

## 3-1. n8n 없이 바로 상담받기 (무료 · MVP)

n8n·DB를 아직 안 붙였을 때 사용할 수 있는 자동 접수·고객 직접 전달 경로가 있습니다.

### (A) 고객이 직접 전달 — 설정 0
자동 전송이 실패하면 완료 화면에 **다시 시도 · 📞 전화 · ✉️ 문자 · 내용 복사** 경로가 나옵니다.
최신 문의 1건은 현재 탭 메모리에만 임시 보관되며 새로고침하거나 탭을 닫으면 사라집니다.
전화·문자·복사는 고객이 직접 실행해야 하므로, 실행하지 않고 화면을 닫은 문의까지 전달된다고 보장할 수는 없습니다.
...
2. `data/config.json` 의 `forms` 를 설정:
   ...
   ```
3. 저장하면 폼 제출 시 요약이 대표 메일로 전송됩니다.
   - 지원 provider는 정확히 `web3forms`, `generic`, `formspree`입니다.
   - Formspree는 `provider: "formspree"`와 해당 endpoint를 함께 설정합니다. endpoint만 바꾸고 provider를 그대로 두면 안 됩니다.
   - 호환 JSON 응답을 직접 제공하는 폼 서비스는 `provider: "generic"`으로 설정합니다.

> 우선순위: 활성화된 n8n → 활성화되고 provider가 지원되는 forms → (둘 다 없으면) 고객 직접 전송(A).
...
2. 대표가 n8n 알림의 승인 링크 또는 별도로 보호된 외부 운영 도구에서 **승인/보류/거절**을 선택합니다.
3. 승인 시 Wait 노드가 재개되고, **승인된 건만** 고객에게 안내가 발송됩니다.

> 공개 관리자 화면(`admin.html`)에는 로컬 문의 개인정보 보드·시드·승인 버튼이 없습니다. 외부 접수 경로 상태와 비개인 사이트 콘텐츠 편집기만 제공합니다.
> 상담 내용은 설정된 n8n/폼 서비스에서 확인합니다. 현장 앱으로 옮길 필요가 있는 접수는 외부 수신 내용을 확인한 뒤 인증된 내부 운영 절차에서 등록하며, 공개 관리자 페이지가 개인정보를 URL 조각이나 브라우저 저장소로 인계하지 않습니다.

---

## 5-1. 알림톡 자동 발송 노드 설정 (완전 자동 발송)

승인된 안내 발송은 **승인된 외부 워크플로** 또는 별도로 **보호된 운영 도구**에서 수행합니다. 공개 관리자(`admin.html`)는 복사·문자·카카오 발송 기능을 제공하지 않습니다.
승인된 건을 **카카오 알림톡으로 자동 발송**하려면 `manmul-inquiry` 워크플로의
...
| 단계 | 목표 | 이 저장소의 대응 |
|------|------|------------------|
| 1단계 | 영업 MVP | 홈·시공사례·단계형 문의·외부 접수 경로 상태·비개인 콘텐츠 편집 ✅ |
