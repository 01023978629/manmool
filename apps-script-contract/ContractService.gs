...
var CT_WARRANTY = [{ name: '방수 관련', months: 36 }, { name: '급배수 등 설비', months: 24 }, { name: '그 밖의 마감공사', months: 12 }];
var CT_WARRANTY_LABEL = '방수 관련 3년 · 급배수 등 설비 2년 · 그 밖의 마감 1년';
...
// contract-backend/src/standard-contract.mjs 에 같은 본문이 있었으나 그 서버는
// 은퇴했고 되살리지 않기로 했다(AGENTS.md). 2026-08-09 하자보증 법정기간
// 정정부터는 **여기가 유일한 정본**이고 저쪽은 옛 값을 그대로 둔 보관물이다 —
// 저쪽을 고치려 들지 마라. 저쪽 테스트가 옛 문구를 못박고 있어 같이 깨진다.
//
// 값의 출처는 사장님(전병덕)이 확정한 조건이다:
//   지급 50/40/10 · 추가공사는 별도 계약 · 공정 동의 3일
// 하자보증은 대표 확정값이 아니라 **법정 기간 그대로**다(건산법 시행령 별표4:
//   방수 3년 · 급배수 등 설비 2년 · 실내건축 등 마감 1년).
//   법정보다 짧게 적으려면 사유와 하자보수보증 수수료를 계약서에 함께 명시해야 하고
//   (같은 법 시행령 제30조②) 그 요건을 못 갖추면 법원은 법정기간을 적용해 왔다 —
//   줄여 적어도 책임은 그대로이고 홈페이지(3년 안내)와 어긋나는 손해만 남는다.
...
function ctStandardBody_(o) {
  ...
  var clauses = [
    ...
    { no: 7, title: '하자보증',
      text: '① 을은 목적물 인도일부터 방수 관련 공사는 3년, 급배수 등 설비공사는 2년, 그 밖의 마감공사는 1년 동안 하자보수 책임을 진다(건설산업기본법 시행령 별표4 기준).\n'
    ...
    // 배상 한도는 **자기 조를 가져야 한다.**
    // 처음에는 제10조(안전 및 민원) 끝에 붙어 있었다. 문구 자체는 일반 배상을
    // 가리켰지만, 안전·민원 조항 안에 있으면 "그 한도는 민원 대응에 관한 것"이라는
    // 반론이 선다. 계약서 해석은 다툼이 있을 때 작성자에게 불리하게 기울고,
    // 이 계약서의 작성자는 을(만물인테리어)이다 — 우리에게 가장 값진 조항을
    // 남의 조항 안에 숨겨 두면 안 된다.
    { no: 11, title: '손해배상의 범위와 한도',
      text: '① 을의 통상 과실로 발생한 재산상 손해에 대한 배상책임은 사고당 금 15,000,000원을 한도로 한다.\n'
        + '② 다음은 제1항의 한도를 적용하지 아니한다.\n'
        + '  1. 을의 고의 또는 중대한 과실로 인한 손해\n'
        + '  2. 생명·신체에 대한 손해\n'
        + '  3. 그 밖에 법령상 책임을 제한할 수 없는 경우\n'
        + '③ 완성작업위험 등 보험의 가입 및 보장 여부는 견적서 또는 특약에 별도로 표시한다.' },

    { no: 12, title: '전자계약의 성립',
      text: '본 계약은 전자문서로 작성되며, 갑의 전자서명·동의기록 및 문서해시로 그 성립과 내용을 증명한다. '
        + '양 당사자는 전자적 방법에 의한 계약 체결에 동의한다.' },

    { no: 13, title: '계약의 해제와 분쟁',
