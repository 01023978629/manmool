...
test('paid leak diagnosis sends purpose, optional preference and immutable inquiry-only once', async()=>{
  ...
  assert.equal(posted[0].inquiryPurpose,'paid-device-diagnosis'); assert.equal(posted[0].preferredVisitDate,'2026-09-15'); assert.equal(posted[0].preferredVisitWindow,'afternoon'); assert.equal(posted[0].bookingStatus,'inquiry-only'); assert.equal(posted[0].ctaId,'leak-inquiry-submit'); assert.equal(posted[0].privacyConsent,true); assert.match(posted[0].message,/\uAC1C\uC778\uC815\uBCF4 \uC218\uC9D1\u00B7\uC774\uC6A9 \uB3D9\uC758: \uB3D9\uC758/);
...
test('phone consultation defaults optional schedule safely and remains inquiry-only',async()=>{
  const page=await newPage();const posted=[];await page.route('https://api.web3forms.com/**',r=>{posted.push(JSON.parse(r.request().postData()));return r.fulfill({status:200,contentType:'application/json',body:'{"success":true}'});});await page.goto(`${origin}/leak.html`);await fillLeak(page);await page.click('#lkSubmit');await waitFor(()=>posted.length===1);assert.equal(posted[0].inquiryPurpose,'phone-consult');assert.equal(posted[0].preferredVisitDate,'');assert.equal(posted[0].preferredVisitWindow,'any');assert.equal(posted[0].bookingStatus,'inquiry-only');assert.equal(posted[0].privacyConsent,true);assert.match(posted[0].message,/\uAC1C\uC778\uC815\uBCF4 \uC218\uC9D1\u00B7\uC774\uC6A9 \uB3D9\uC758: \uB3D9\uC758/);await page.close();
});
