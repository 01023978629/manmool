/* build-pages-artifact.mjs가 만든 실제 GitHub Pages 산출물을 검증한다. */
import fs from 'node:fs';
import path from 'node:path';
import crypto from 'node:crypto';
import { fileURLToPath } from 'node:url';
import { PORTAL_PUBLIC_FILES, expectedPublicFiles, toPublicPath } from './pages-artifact-policy.mjs';

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const SITE = path.join(ROOT, '_site');
const fail = [];
const googleVerificationFile = 'google11dc37fbc3ab6e98.html';
const forbiddenTop = new Set(['.git', '.github', '.claude', '.codex', 'apps-script-contract', 'contract-backend', 'docs', 'integrations', 'scripts']);
const textFile = /\.(?:html|css|js|json|txt|xml)$/i;
const secretIdentifiers = [/\b(?:APP_TOKEN|OFFICE_SESSION_SECRET|pinHash|pinSalt)\b/];
const portalFixtureMarkers = [
  /\b(?:123456|session-allowlist-test|session-workflow|session-test|test-session|fixture-(?:session|contact|request))\b/i,
  /010-(?:1234|9999)-(?:5678|8888)/,
  /(?:김소장|홍길동|대표 내부 메모)/,
];

function sha256(file) {
  return crypto.createHash('sha256').update(fs.readFileSync(file)).digest('hex');
}

function walk(directory) {
  const files = [];
  const directories = [];
  function visit(current) {
    for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
      const absolute = path.join(current, entry.name);
      if (entry.isDirectory()) {
        directories.push(absolute);
        visit(absolute);
      } else if (entry.isFile()) {
        files.push(absolute);
      } else {
        fail.push(`산출물에 허용되지 않은 특수 경로가 있음: ${toPublicPath(path.relative(SITE, absolute))}`);
      }
    }
  }
  visit(directory);
  return { files, directories };
}

function expectedDirectories(files) {
  const directories = new Set();
  for (const { relative } of files) {
    const parts = relative.split('/');
    parts.pop();
    while (parts.length) {
      directories.add(parts.join('/'));
      parts.pop();
    }
  }
  return directories;
}

if (!fs.existsSync(SITE)) {
  console.error('✗ _site 허용목록 산출물이 없습니다 — build-pages-artifact.mjs를 먼저 실행해야 합니다');
  process.exit(1);
}

let expected = [];
try {
  expected = expectedPublicFiles(ROOT);
} catch (error) {
  fail.push(`공개 소스 허용목록 오류: ${error.message}`);
}
const actual = walk(SITE);
const actualFiles = new Map(actual.files.map((file) => [toPublicPath(path.relative(SITE, file)), file]));
const expectedFiles = new Map(expected.map((entry) => [entry.relative, entry.source]));

for (const [relative, source] of expectedFiles) {
  const output = actualFiles.get(relative);
  if (!output) fail.push(`필수 공개 산출물 누락: ${relative}`);
  else if (sha256(source) !== sha256(output)) fail.push(`산출물이 현재 공개 소스와 다름(신선한 빌드 필요): ${relative}`);
}
for (const relative of actualFiles.keys()) {
  if (!expectedFiles.has(relative)) fail.push(`허용목록 밖의 output-only 산출물이 포함됨: ${relative}`);
}

const actualDirectories = new Set(actual.directories.map((directory) => toPublicPath(path.relative(SITE, directory))));
const expectedDirs = expectedDirectories(expected);
for (const relative of expectedDirs) if (!actualDirectories.has(relative)) fail.push(`필수 공개 폴더 누락: ${relative}`);
for (const relative of actualDirectories) if (!expectedDirs.has(relative)) fail.push(`허용목록 밖의 output-only 폴더가 포함됨: ${relative}`);
for (const entry of fs.readdirSync(SITE, { withFileTypes: true })) {
  if (forbiddenTop.has(entry.name)) fail.push(`비공개 소스가 Pages 산출물에 포함됨: ${entry.name}`);
}

for (const [relative, file] of actualFiles) {
  if (!textFile.test(file)) continue;
  const content = fs.readFileSync(file, 'utf8');
  if (secretIdentifiers.some((marker) => marker.test(content))) fail.push(`공개 산출물에 비밀 식별자가 있음: ${relative}`);
  if (PORTAL_PUBLIC_FILES.includes(relative) && portalFixtureMarkers.some((marker) => marker.test(content))) {
    fail.push(`포털 산출물에 테스트·PIN·세션·연락처 fixture가 있음: ${relative}`);
  }
}

const googleVerificationPath = path.join(SITE, googleVerificationFile);
if (fs.existsSync(googleVerificationPath)) {
  const actualVerification = fs.readFileSync(googleVerificationPath, 'utf8').trim();
  if (actualVerification !== `google-site-verification: ${googleVerificationFile}`) {
    fail.push('Google Search Console 소유권 확인 파일 내용이 발급값과 다름');
  }
}

function checkRef(owner, raw) {
  const ref = String(raw || '').trim();
  if (!ref || /^(?:#|https?:|tel:|sms:|mailto:|data:|blob:|javascript:)/i.test(ref)) return;
  const clean = ref.split('#')[0].split('?')[0];
  if (!clean || clean.startsWith('/')) return;
  let target = path.resolve(path.dirname(owner), decodeURIComponent(clean));
  if (clean.endsWith('/')) target = path.join(target, 'index.html');
  const relative = path.relative(SITE, target);
  if (relative.startsWith('..') || path.isAbsolute(relative)) fail.push(`산출물 밖을 가리키는 링크: ${toPublicPath(path.relative(SITE, owner))} -> ${ref}`);
  else if (!fs.existsSync(target)) fail.push(`산출물에서 끊긴 링크: ${toPublicPath(path.relative(SITE, owner))} -> ${ref}`);
}

for (const file of actual.files) {
  if (/\.html$/i.test(file)) {
    const source = fs.readFileSync(file, 'utf8');
    for (const match of source.matchAll(/\b(?:href|src)=["']([^"']+)["']/gi)) checkRef(file, match[1]);
  } else if (/\.css$/i.test(file)) {
    const source = fs.readFileSync(file, 'utf8');
    for (const match of source.matchAll(/url\(\s*["']?([^"')]+)["']?\s*\)/gi)) checkRef(file, match[1]);
  }
}

if (fail.length) {
  console.error(`✗ Pages 허용목록 산출물 ${fail.length}건 문제\n`);
  fail.slice(0, 80).forEach((item) => console.error(`  - ${item}`));
  if (fail.length > 80) console.error(`  - 외 ${fail.length - 80}건`);
  process.exit(1);
}

console.log(`✓ Pages 명시 허용목록 산출물 정상 — 파일 ${actual.files.length}개 · source/output 정확 일치`);
